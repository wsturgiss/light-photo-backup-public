...
@Dao interface BackupRecordDao {
 @Insert(onConflict=OnConflictStrategy.IGNORE) suspend fun insert(records:List<BackupRecord>):List<Long>
  ...
  @Query("SELECT lastErrorMessage FROM backup_records WHERE status='RETRYABLE_FAILURE' AND nextRetryMillis>:now ORDER BY nextRetryMillis LIMIT 1") suspend fun nextDelayedRetryError(now:Long):String?
