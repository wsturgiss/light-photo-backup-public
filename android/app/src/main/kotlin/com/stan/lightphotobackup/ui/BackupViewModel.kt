...
class BackupViewModel(app:Application):AndroidViewModel(app){
 ...
 fun configureImmich(baseUrl:String,apiKey:String){viewModelScope.launch{Log.i("PhotoBackupImmich","validating server");runCatching{c.immich.configure(baseUrl,apiKey)}.onSuccess{c.settings.lockProvider();transient.value=transient.value.copy(error=null);Log.i("PhotoBackupImmich","server validation succeeded")}.onFailure{transient.value=transient.value.copy(error=it.message?:"Unable to connect to Immich");Log.w("PhotoBackupImmich","server validation failed type=${it.javaClass.simpleName}")}}}
 fun retry(){viewModelScope.launch{c.database.records().retryFailed();backUpNow()}}
 fun disconnect(){viewModelScope.launch{when(state.value.provider){BackupProvider.GOOGLE_PHOTOS->c.pairing.disconnect();BackupProvider.IMMICH->c.immich.disconnect()};transient.value=TransientState()}}
}
