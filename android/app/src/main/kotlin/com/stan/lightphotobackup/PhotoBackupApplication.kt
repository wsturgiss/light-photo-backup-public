...
import com.stan.lightphotobackup.immich.*
...
class AppContainer(app:Application){val database=BackupDatabase.create(app);val settings=SettingsRepository(app);private val http=OkHttpClient.Builder().connectTimeout(15,TimeUnit.SECONDS).readTimeout(60,TimeUnit.SECONDS).writeTimeout(2,TimeUnit.MINUTES).callTimeout(3,TimeUnit.MINUTES).build();private val credentials=SecureCredentialStore(app);val pairing=PairingRepository(PairingApi(http),credentials);val immich=ImmichRepository(ImmichApi(app.contentResolver,http),credentials);val coordinator=BackupCoordinator(app,database,CameraMediaRepository(app),{provider->when(provider){BackupProvider.GOOGLE_PHOTOS->BackupCredential.Google(pairing.accessToken());BackupProvider.IMMICH->immich.backupCredential()}},mapOf(BackupProvider.GOOGLE_PHOTOS to GooglePhotosUploader(app.contentResolver,http),BackupProvider.IMMICH to ImmichApi(app.contentResolver,http)),settings)}
