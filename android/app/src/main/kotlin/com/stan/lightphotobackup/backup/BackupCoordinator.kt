...
class BackupCoordinator(private val context:Context,private val db:BackupDatabase,private val media:CameraMediaRepository,private val credential:(suspend (BackupProvider)->BackupCredential),private val uploaders:Map<BackupProvider,BackupUploader>,private val settings:SettingsRepository){
 ...
 suspend fun run(): BackupResult = mutex.withLock { withContext(Dispatchers.IO) {
  ...
  if(photos.isNotEmpty()) db.records().markMissing(photos.map { it.uri.toString() },now)
   var uploaded=0; var failed=0;var retryNeeded=false;var authorizationExpired=false;var lastError:String?=null
  ...
  while(currentCoroutineContext().isActive) {
   ...
   try {
    ...
    } catch(e:com.stan.lightphotobackup.pairing.AuthorizationExpiredException) { lastError="Account connection expired";db.records().updateState(item.id,BackupStatus.RETRYABLE_FAILURE,errorCode="authorization_expired",errorMessage=lastError,now=System.currentTimeMillis());failed++;authorizationExpired=true;break
    } catch(e:java.io.FileNotFoundException) { lastError="Local photo is no longer available";db.records().updateState(item.id,BackupStatus.MISSING_LOCAL_FILE,errorCode="local_missing",errorMessage=lastError,now=System.currentTimeMillis()); failed++
    } catch(e:UploadException) { lastError=e.message;val delay=e.retryAfterMillis?:((1L shl item.attemptCount.coerceAtMost(8))*30_000L).coerceAtMost(6*60*60_000L); db.records().updateState(item.id,if(e.retryable)BackupStatus.RETRYABLE_FAILURE else BackupStatus.PERMANENT_FAILURE,attemptDelta=1,attemptAt=System.currentTimeMillis(),retryAt=if(e.retryable)System.currentTimeMillis()+delay else null,errorCode=e.code,errorMessage=e.message.take(160),now=System.currentTimeMillis()); failed++;retryNeeded=e.retryable; if(e.retryable) break
    } catch(e:Exception) { lastError="Temporary backup error";if(BuildConfig.DEBUG)Log.w("PhotoBackupWorker","item failed category=temporary type=${e.javaClass.simpleName}");db.records().updateState(item.id,BackupStatus.RETRYABLE_FAILURE,attemptDelta=1,attemptAt=System.currentTimeMillis(),retryAt=System.currentTimeMillis()+60_000,errorCode="temporary",errorMessage=lastError,now=System.currentTimeMillis()); failed++;retryNeeded=true; break }
  }
  val end=System.currentTimeMillis(); if(uploaded>0)settings.success(end)
    val delayedError=if(provider==BackupProvider.IMMICH)db.records().nextDelayedRetryError(end) else null
    db.summary().save(BackupSummary(lastScanMillis=now,lastAttemptMillis=end,lastSuccessMillis=if(uploaded>0)end else null,discovered=inserted,uploaded=uploaded,failed=failed,operation="Idle",lastError=if(authorizationExpired)"Account connection expired" else if(provider==BackupProvider.IMMICH)lastError?:delayedError?:if(failed>0)"Backup needs attention" else null else if(failed>0)"Backup needs attention" else null))
